...
const userProfileSchema: TableSchema = {
  ...
  columns: [
    ...
    { name: "user_id", type: "TEXT", notNull: true, unique: true, references: { table: "users", column: "id" } },
 ...
 */
function renameTableIfExists(db: Database, oldName: string, newName: string): boolean {
    try {
...
async function migrateUserRoles(db: Database) {
    console.log('🔄 [Paso Adicional] Migrando asignaciones de roles de usuario...');

    // 1. Crear un mapa de los IDs de los roles VIEJOS a sus NOMBRES.
    //    Ej: Map { 1 => 'admin', 2 => 'user' }
    const oldRoles = db.query("SELECT idRol, nombreRol FROM roles_legacy").all() as { idRol: number | string; nombreRol: string }[];
    const oldRoleIdToNameMap = new Map(oldRoles.map(r => [r.idRol, r.nombreRol]));
    
    // 2. Crear un mapa de los NOMBRES de los roles NUEVOS a sus IDs.
    //    Ej: Map { 'admin' => 'xyz...', 'user' => 'abc...' }
    const newRoles = db.query("SELECT id, name FROM roles").all() as { id: string; name: string }[];
    const newRoleNameToIdMap = new Map(newRoles.map(r => [r.name.toLowerCase(), r.id]));

    // 3. Obtener los usuarios viejos con su ID de usuario y su ID de rol (legacy).
    const oldUsersWithRoles = db.query("SELECT idUsuario, rolUsuario FROM usuarios_legacy WHERE rolUsuario IS NOT NULL").all() as { idUsuario: string; rolUsuario: number | string }[];
    ...
    const insertStmt = db.prepare("INSERT OR IGNORE INTO user_roles (user_id, role_id) VALUES (?, ?)");
    let count = 0;
    let notFoundCount = 0;
    ...
    for (const oldUser of oldUsersWithRoles) {
        ...
        if (roleName) {
            ...
            if (newRoleId) {
                ...
                console.warn(`   -> Advertencia: No se encontró el rol '${roleName}' en la nueva tabla de roles (para el usuario ID '${oldUser.idUsuario}').`);
                notFoundCount++;
            }
        } else {
             console.warn(`   -> Advertencia: No se encontró un nombre de rol para el ID de rol legacy '${oldUser.rolUsuario}' (para el usuario ID '${oldUser.idUsuario}').`);
             notFoundCount++;
        }
    }
    ...
    if (notFoundCount > 0) {
        console.log(`   -> ⚠️ Hubo ${notFoundCount} asignaciones que no se pudieron migrar por falta de correspondencia.`);
    }
}
 ...
 */
async function main() {
  console.log('🚀 Iniciando el proceso de migración de base de datos (versión completa)...');
  const db = new Database(DATABASE_PATH);
  
  try {
    // --- PASO PREVIO: AISLAR TABLAS LEGACY ---
    console.log('🔍 [Paso 1/6] Buscando y aislando tablas legacy...');
    const usuariosLegacyExists = renameTableIfExists(db, 'usuarios', 'usuarios_legacy');
    const rolesLegacyExists = renameTableIfExists(db, 'roles', 'roles_legacy');

    // --- PASO 2: INICIALIZAR ESQUEMA (LIBRERÍA + EXTENDIDO) ---
    console.log('✅ [Paso 2/6] Inicializando esquema de la librería y tablas personalizadas...');
    ...
    if (!initResult.success) {
      throw new Error(`Error fatal inicializando el esquema: ${initResult.errors.join(', ')}`);
    }
    console.log(`   -> Esquema listo. Tablas creadas: ${initResult.tablesCreated.join(', ')}`);

    // --- PASO 3: SEMBRAR DATOS POR DEFECTO ---
    console.log('🌱 [Paso 3/6] Sembrando datos por defecto (roles, permisos)...');
    ...
    if (usuariosLegacyExists) {
      ...
      console.log('   -> Migración preparada para: usuarios_legacy -> user_profiles');
    }
    ...
    if (migrationsToRun.length === 0) {
      console.log('🟢 [Paso 4/6] No se encontraron tablas legacy para migrar.');
        ...
        if (!migrationResult.success) {
            throw new Error(`Fallo en la migración de datos: ${migrationResult.errors.join(', ')}`);
        }
    }
    ...
    console.error('💥 Ha ocurrido un error catastrófico durante la migración:', error.message);
  } finally {
