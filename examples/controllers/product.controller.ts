...
import { DatabaseInitializer, BaseController } from '../../dist/index';
...
export class ProductController {
  ...
  constructor(public dbInitializer: DatabaseInitializer) {
    this.productController = dbInitializer.createController<Product>('products');
    this.categoryController = dbInitializer.createController<Category>('categories');
  }
  ...
  async getProducts(c: Context) {
    try {
      ...
      const filter = categoryId ? { category_id: categoryId, is_available: true } : { is_available: true };
  ...
  async getProduct(c: Context) {
    try {
      ...
      const result = await this.productController.findByIdWithRelations(id, [categoryJoin]);
  ...
  async createProduct(c: Context) {
    try {
      ...
      if (body.category_id) {
        const category = await this.categoryController.findById(body.category_id);
        if (!category) {
          return c.json({
            success: false,
      ...
      const productWithCategory = await this.productController.findByIdWithRelations(
  ...
  async updateProduct(c: Context) {
    try {
      ...
      const allowedFields = ['name', 'description', 'price', 'category_id', 'image', 'fallback', 'is_available', 'stock_quantity'];
  ...
  async getCategories(c: Context) {
    try {
      const categoriesResult = await this.categoryController.findAll({ where: { is_active: true } });
      const productsResult = await this.productController.findAll({ where: { is_available: true } });
  ...
  async createCategory(c: Context) {
    try {
      ...
      const existingCategoriesResult = await this.categoryController.findAll({ where: { name: body.name } });
      ...
      const createdCategory = await this.categoryController.create(categoryData);
