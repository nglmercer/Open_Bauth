...
describe('PermissionService', () => {
  ...
  test('should handle the full RBAC flow correctly', async () => {
    // 1. Crear un Rol
    const roleResult = await permissionService.createRole({ name: 'editor', description: 'Can edit content' });
    ...
    const assignResult = await permissionService.assignPermissionToRole(editorRole.id, editPerm.id);
    expect(assignResult.success).toBe(true);

    // 4. Crear un Usuario
    const userReg = await authService.register({ email: 'editor@test.com', password: 'password' });
    const testUser = userReg.user!;

    // 5. Asignar Rol al Usuario
    const assignUserRoleResult = await authService.assignRole(testUser.id, 'editor');
    expect(assignUserRoleResult.success).toBe(true);

    // 6. Verificar permisos (la prueba de fuego)
    const hasPerm = await permissionService.userHasPermission(testUser.id, 'edit-article');
    expect(hasPerm).toBe(true);
    
    const canAccess = await permissionService.userCanAccessResource(testUser.id, 'article', 'edit');
    expect(canAccess).toBe(true);

    // 7. Verificar un permiso que NO tiene
    const hasWrongPerm = await permissionService.userHasPermission(testUser.id, 'delete-article');
  ...
  test('should delete a role and its assignments', async () => {
    ...
    const role = (await permissionService.createRole({ name: 'temporary-role' })).role!;
    const user = (await authService.register({ email: 'temp@user.com', password: 'pw' })).user!;
    ...
    const findDeletedRole = await permissionService.findRoleByName('temporary-role');
    expect(findDeletedRole).toBeNull();

    // Verificación: El usuario ya no tiene ese rol
    const userHasRole = await permissionService.userHasRole(user.id, 'temporary-role');
    expect(userHasRole).toBe(false);
  });

  test('userHasRole should return false for a non-existent role', async () => {
    const user = (await authService.register({ email: 'user@test.com', password: 'pw' })).user!;
    
    const hasRole = await permissionService.userHasRole(user.id, 'non-existent-role');
