...
describe('JWTService', () => {
  ...
  describe('Constructor', () => {
    ...
    test('should throw error with undefined secret', () => {
      expect(() => new JWTService(undefined as any)).toThrow('JWT secret is required');
    });
  });
  ...
  describe('Token Verification', () => {
    ...
    test('should handle malformed JSON in payload', async () => {
      ...
      const signature = await jwtService['createSignature'](`${header}.${payload}`);
  ...
  describe('Utility Methods', () => {
    ...
    test('should handle invalid expiration formats', () => {
      expect(() => jwtService['parseExpirationTime']('invalid')).toThrow('Invalid expiration format');
      expect(() => jwtService['parseExpirationTime']('1x')).toThrow('Invalid expiration format');
      expect(() => jwtService['parseExpirationTime']('')).toThrow('Invalid expiration format');
    });
  });
